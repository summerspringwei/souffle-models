...
if [ -n "${SOUFFLE_RUN}" ] && [ "${SOUFFLE_RUN}" = "TRUE" ]; then
ncu --clock-control none -o ncu-souffle_efficientnet_O3 -f --target-processes all python3 souffle_efficientnet.py O3 1 1
...
cd ${DIRPATH}/mmoe
if [ -n "${SOUFFLE_RUN}" ] && [ "${SOUFFLE_RUN}" = "TRUE" ]; then
ncu --clock-control none --set full -o ncu-souffle_mmoe_O2 -f --target-processes all python3 souffle_mmoe.py O2 1 1
