...
#   ncu --clock-control none --set full -o ncu-souffle_bert_O4 -f --target-processes all python3 souffle_bert.py O4 1 1
...
# cd ${DIRPATH}/resnext
# if [ -n "${SOUFFLE_RUN}" ] && [ "${SOUFFLE_RUN}" = "TRUE" ]; then
# ncu --clock-control none --set full -o ncu-souffle_resnext_O4 -f --target-processes all \
...
cd ${DIRPATH}/lstm
if [ -n "${SOUFFLE_RUN}" ] && [ "${SOUFFLE_RUN}" = "TRUE" ]; then
ncu --clock-control none --set full -o ncu-souffle_lstm_O4 -f --target-processes all \
...
if [ -n "${SOUFFLE_RUN}" ] && [ "${SOUFFLE_RUN}" = "TRUE" ]; then
ncu --clock-control none --set full -o ncu-souffle_efficientnet_O3 -f --target-processes all python3 souffle_efficientnet.py O3 1 1
...
if [ -n "${SOUFFLE_RUN}" ] && [ "${SOUFFLE_RUN}" = "TRUE" ]; then
ncu --clock-control none  -o ncu-souffle_swin_trans_O4 -f --target-processes all python3 souffle_swin_trans.py O4 1 1
...
cd ${DIRPATH}/mmoe
if [ -n "${SOUFFLE_RUN}" ] && [ "${SOUFFLE_RUN}" = "TRUE" ]; then
ncu --clock-control none --set full -o ncu-souffle_mmoe_O4 -f --target-processes all python3 souffle_mmoe.py O4 1 1
ncu -i ./ncu-souffle_mmoe_O4.ncu-rep --csv --page raw  | grep -v "at::native*" > ncu-souffle_mmoe_O4.csv
fi
SOUFFLE_MMoE_LATENCY=$(python3 ../../extract_ncu_cuda_kernel_latency.py ncu-souffle_mmoe_O4.csv)
