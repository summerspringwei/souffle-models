...
cd ${DIRPATH}/lstm
if [ -n "${SOUFFLE_RUN}" ] && [ "${SOUFFLE_RUN}" = "TRUE" ]; then
ncu --clock-control none --set detailed -o ncu-souffle_lstm_O4 -f --target-processes all \
...
cd ${DIRPATH}/mmoe
if [ -n "${SOUFFLE_RUN}" ] && [ "${SOUFFLE_RUN}" = "TRUE" ]; then
ncu --clock-control none --set detailed -o ncu-souffle_mmoe_O2 -f --target-processes all python3 souffle_mmoe.py O2 1 1
