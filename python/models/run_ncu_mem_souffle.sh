...
SOUFFLE_BERT_NUM_KERNELS=$(wc -l ncu-${NAME}.csv | awk '{ print $1 }')
...
SOUFFLE_RESNEXT_NUM_KERNELS=$(wc -l ncu-${NAME}.csv | awk '{ print $1 }')
...
SOUFFLE_LSTM_NUM_KERNELS=$(wc -l ncu-${NAME}.csv | awk '{ print $1 }')

# EfficientNet Pass
cd ${DIRPATH}/efficientnet
NAME="souffle_efficientnet_O3"
if [ -n "${SOUFFLE_RUN}" ] && [ "${SOUFFLE_RUN}" = "TRUE" ]; then
ncu --clock-control none -o ncu-${NAME} -f --target-processes all python3 souffle_efficientnet.py O3 1 1
ncu -i ./ncu-${NAME}.ncu-rep --csv --page raw | grep -v "at::native" > ncu-${NAME}.csv
fi
SOUFFLE_EFFICIENTNET_MEM=$(python3 ../../extract_ncu_cuda_mem_read.py ncu-${NAME}.csv)
SOUFFLE_EFFICIENTNET_NUM_KERNELS=$(wc -l ncu-${NAME}.csv | awk '{ print $1 }')

# SwinTrans. Pass
cd ${DIRPATH}/swin_transformer
NAME="souffle_swin_trans_O4"
if [ -n "${SOUFFLE_RUN}" ] && [ "${SOUFFLE_RUN}" = "TRUE" ]; then
ncu ${NCU_ARGS} -o ncu-${NAME} -f --target-processes all python3 souffle_swin_trans.py O4 1 1
ncu -i ./ncu-${NAME}.ncu-rep --csv --page raw  | grep -v "at::native" > ncu-${NAME}.csv
fi
SOUFFLE_SWIN_TRANS_MEM=$(python3 ../../extract_ncu_cuda_mem_read.py ncu-${NAME}.csv)
SOUFFLE_SWIN_TRANS_NUM_KERNELS=$(wc -l ncu-${NAME}.csv | awk '{ print $1 }')

# MMOE
cd ${DIRPATH}/mmoe
NAME="souffle_mmoe_O2"
if [ -n "${SOUFFLE_RUN}" ] && [ "${SOUFFLE_RUN}" = "TRUE" ]; then
ncu ${NCU_ARGS} -o ncu-${NAME} -f --target-processes all python3 souffle_mmoe.py O2 1 1
ncu -i ./ncu-${NAME}.ncu-rep --csv --page raw  | grep -v "at::native*" > ncu-${NAME}.csv
fi
SOUFFLE_MMoE_MEM=$(python3 ../../extract_ncu_cuda_mem_read.py ncu-${NAME}.csv)
SOUFFLE_MMoE_NUM_KERNELS=$(wc -l ncu-${NAME}.csv | awk '{ print $1 }')
cd ${DIRPATH}
